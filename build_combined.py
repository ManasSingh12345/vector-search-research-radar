...
function renderRadar(){
  ...
  host.innerHTML=clusters.map((c,i)=>{
    ...
    return `<div class="cluster" data-i="${i}"><button class="crow" aria-expanded="false"><span class="rank">${String(i+1).padStart(2,"0")}</span>
      ...
      <div class="papers">${papers.map(paperRow).join("")}${c.all.length>40?`<div class="ptoggle" style="padding-top:10px">+${c.all.length-40} more in DB</div>`:""}</div></div>`;}).join("");
  host.querySelectorAll(".cluster").forEach(el=>{const btn=el.querySelector(".crow");btn.addEventListener("click",()=>{const o=el.classList.toggle("open");btn.setAttribute("aria-expanded",o?"true":"false");});});
